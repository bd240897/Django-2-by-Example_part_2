...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid():
            ...
            # создаем новые доп сведения для пользователя
...
@login_required
def edit(request):
    """Запись доп свдеений для пользователя"""

    if request.method == 'POST':
        # что такое instance???
        # для базовых сведений о  пользователе
        user_form = UserEditForm(instance=request.user, data=request.POST)
        # для дополнительной, расширенной информации
        profile_form = ProfileEditForm(instance=request.user.profile, data=request.POST, files=request.FILES)
        if user_form.is_valid() and profile_form.is_valid():
            user_form.save()
            profile_form.save()
            # систему уведомлений - переменная передается в контекст автоматически
